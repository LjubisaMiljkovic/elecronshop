    ...
    return (
        <div className='bg-grayLigth py-[20px]'>
            <div className='container mx-auto flex flex-col md:flex-row items-center gap-[20px] '>
                <button className='px-[24px] py-[12px] bg-mainBlue rounded-lg text-mainYellow'
                    onClick={showCategoryHandler}>Category</button>

                <ul className='grid grid-cols-1 place-items-center md:grid-cols-3 lg:grid-cols-5 gap-5 w-[100%]'>
                    {isActive && category.map((cat, index) => {
                        return <li key={index} className='bg-blue-300 px-[16px] py-[8px] w-[250px] text-white hover:bg-mainBlue rounded-lg
                        hover:text-white text-center cursor-pointer transition-all' onClick={() => handleCategory(cat)}>{cat}</li>
                    })}
                </ul>
            </div>
        </div>
